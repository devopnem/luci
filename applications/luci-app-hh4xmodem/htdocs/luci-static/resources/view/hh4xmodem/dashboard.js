...
function renderStatusTab(ctx) {
	var signal = ctx.signal, status = ctx.status, conn = ctx.conn, network = ctx.network;
	var sim = ctx.sim, sysinfo = ctx.sysinfo, simPinLocked = ctx.simPinLocked,
...
function renderInfoTab(ctx) {
	var signal = ctx.signal, status = ctx.status, conn = ctx.conn,
		network = ctx.network, sim = ctx.sim, sysinfo = ctx.sysinfo,
		simPinLocked = ctx.simPinLocked, simReady = ctx.simReady,
		simPinSet = ctx.simPinSet, lan = ctx.lan, upnp = ctx.upnp,
		lang = ctx.lang;
					...
					'click': function (e) {
						...
						callUnlockSim({ code: code, lock_state: sim.SIMLockState || 0 }).then(function (r) {
							if (r && r.error == null) {
								...
								btn.disabled = false;
								ui.addNotification(null, E('p', [_('Unlock command sent. Verifying SIM state...')]), 'info');
								setTimeout(function () { window.location.reload(); }, 3000);
							}
						}).catch(function (err) {
